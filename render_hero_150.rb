# =============================================================================
# Rumicarts — HERO Render — 150 cm RAFLI KOPYASI (SketchUp 2026 + V-Ray 7)
#
# ⚠️ BU, render_hero.rb'NİN AYRI KOPYASIDIR. Orijinal (100 rafsız) pipeline'a
# DOKUNMAZ: ayrı modül (Rumicarts150), ayrı çıktı klasörü (renders150). Eskiyi
# bozmadan 150 raflı aracın render setini almak için yazıldı (2026-06-19).
#
# Sahnede 3 araç var (150 raflı / 150 rafsız / 100 raflı). Mevcut 7 sahne kamerası
# ESKİ araca (100 rafsız) bakıyor. Bu kopya, kameraları 150 raflı aracın merkezine
# YENİDEN NİŞANLAR (aim_at_selection) — geometriyi taşımadan, açı/uzaklık/fov koruyarak.
#
# KULLANIM (Ruby Console, bu dosyayı load et):
#   1) 150 RAFLI aracı (tek grup/component) TIKLA → Rumicarts150::Hero.aim_at_selection
#   2) Diğer 2 aracı seç → H (Hide) ile gizle. Geri: Edit > Unhide All.
#   3) V-Ray Render bas → 150 raflı görününce Stop (ISIT).
#   4) Doğrula: Rumicarts150::Hero.run(test: true) → render_hero_test.bat → ON karesine bak.
#   5) Doğruysa: Rumicarts150::Hero.run  (+ run_colors / run_variant aynı offset'le)
#   Çıktılar: C:/Users/kuruyemis/Desktop/renders150
# =============================================================================

module Rumicarts150
  module Hero
    # ---- AYARLAR ------------------------------------------------------------
    OUT_DIR = "C:/Users/kuruyemis/Desktop/renders150"   # AYRI klasör (eskiyi ezmez), otomatik oluşur
    ...
    # ---- 150 RAFLI'YA NİŞAN ALMA (iki-araç offset'i) -----------------------
    # 7 sahne kamerası ESKİ araca (100 rafsız) GÜZEL kadrajla bakıyor (araç kareyi dolduruyor).
    # 150 raflıyı AYNI kadrajla çekmek için kamerayı, iki aracın MERKEZ farkı kadar kaydırırız:
    # offset = merkez(150 raflı) - merkez(100 rafsız). Yön/uzaklık/fov AYNEN korunur; 150 raflı,
    ...
    #
    # KULLANIM (sırayla):
    #   0) Edit > Unhide All (4 araç da görünsün ki ikisini de seçebil).
    #   1) ESKİ 100 RAFSIZ aracı (ilk render'ın aracı) TIKLA → Rumicarts150::Hero.mark_source
    #   2) 150 RAFLI aracı TIKLA → Rumicarts150::Hero.aim_at_selection
    #   3) 150 raflı HARİÇ 3 aracı seç → H (Hide). Geri: Edit > Unhide All.
    #   4) V-Ray ısıt (Render → Stop) → Hero.run(test: true) → render_hero_test.bat → PNG.
    # Sıfırla: Rumicarts150::Hero.aim_reset
    ...
    def self.mark_source
      ...
      puts "[Nisan] SIMDI: 150 RAFLI araci sec → Rumicarts150::Hero.aim_at_selection"
    end
    ...
    def self.aim_at_selection
      ...
      puts format("[Nisan] HEDEF (150 rafli) merkez = (%.0f, %.0f, %.0f)", c.x, c.y, c.z)
      puts format("[Nisan] OFFSET = (%.0f, %.0f, %.0f) inc — 150 rafli, eski aracin kadrajinda gorunecek.", *@offset)
      puts "[Nisan] SIMDI: 150 rafli HARIC 3 araci gizle (H) → V-Ray isit → Hero.run(test: true)."
    end
    ...
    # kutu yüzeyi) TIKLA → Ruby Console'da: Rumicarts150::Hero.body_material
    ...
    # KULLANIM (yansıma testi): Rumicarts150::Hero.set_body_color("kirmizi")
    ...
    def self.cam_transform_line(scene_name)
      ...
      # 150 RAFLI'YA NİŞAN (mark_source + aim_at_selection ile @offset set edildiyse):
      # yön/uzaklık/fov AYNEN korunur, sadece eye = eye + offset (iki araç merkez farkı) →
      # 150 raflı, eski 100 rafsızın durduğu kadrajda aynı boyutta görünür. @offset yoksa dokunma.
      if @offset
    ...
    def self.run(test: false)
      ...
      puts "[Hero] Nişan: #{@offset ? '150 RAFLI (offset AKTİF)' : 'YOK (eski araç!)'}"
...
# 1) Teşhis (kameralar farklı mı?):  Rumicarts150::Hero.check_cameras
# 2) (opsiyonel) API keşfi:           Rumicarts150::Hero.discover
# 3) Çalıştır (vrscene + .bat üret):  Rumicarts150::Hero.run
...
#   a) Rumicarts150::Hero.teker_off            → tekerlek+çamurluğu gizle
#   b) V-Ray Render bas → tekerleksiz araba görününce Stop (sahneyi ısıt)
#   c) Rumicarts150::Hero.run_tekeryok(test: true)  → 1 test karesi → render_tekeryok_test.bat
#   d) Temizse: Rumicarts150::Hero.run_tekeryok     → 30 kare → render_tekeryok.bat
#   e) Rumicarts150::Hero.teker_on             → tekerleği geri getir (gerekirse)
#
# TENTE-YOK + KOMBİNASYON seti (KumasTente tag'i SketchUp'ta hazır olmalı):
#   A) Tente YOK (tekerlek VAR) — +30 kare:
#      1) Rumicarts150::Hero.set_variant_tags(hide: ["KumasTente"])
#      2) V-Ray Render → tentesiz araba görününce Stop (ısıt)
#      3) Rumicarts150::Hero.run_variant(suffix: "-tenteyok", hide: ["KumasTente"], test: true)
#      4) Temizse: Rumicarts150::Hero.run_variant(suffix: "-tenteyok", hide: ["KumasTente"])
#   B) Tente YOK + Tekerlek YOK — +30 kare:
#      1) Rumicarts150::Hero.set_variant_tags(hide: ["KumasTente", "DekorTekerlek"])
#      2) V-Ray Render → Stop (ısıt)
#      3) Rumicarts150::Hero.run_variant(suffix: "-tekeryok-tenteyok", hide: ["KumasTente", "DekorTekerlek"])
#   C) Hepsi bitince: Rumicarts150::Hero.variant_reset  → tüm tag'leri aç + beyaza dön
